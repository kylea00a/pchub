...
powershell -NoProfile -Command "$root=$env:PCHUB_AGENT_ROOT; Get-CimInstance Win32_Process -Filter \"Name='node.exe'\" | Where-Object { $_.CommandLine -and ($_.CommandLine.ToLower().Contains('pchub-host.js') -or $_.CommandLine.ToLower().Contains('agent.cjs')) -and $_.CommandLine.ToLower().Contains($root.ToLower()) } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -EA SilentlyContinue }"
